...
class DiskDatabase(DatabaseInterface):
    ...
    def __init__(self, base_path: Path):
        ...
        self.audit_path = base_path / "audit.jsonl"
        self.outcomes_path = base_path / "outcomes.jsonl"
    ...
    def insert_audit(
        ...
        *,
        ...
        append_jsonl(self.audit_path, doc)
    ...
    def count_events_for_ip_since(self, ip: str, event: str, since_iso: str) -> int:
        ...
        return sum(
            ...
            and str(r.get("created_at", "")) >= since_iso
        )
    ...
    def fetch_ip_for_run(self, run_id: str) -> str | None:
        ...
        records = read_jsonl(self.audit_path)
        for r in records:
            if r.get("run_id") == run_id and r.get("ip"):
                return r["ip"]
        return None
    ...
    def replace_run_outcomes(
        ...
        records = read_jsonl(self.outcomes_path)
        filtered = [r for r in records if r.get("run_id") != run_id]
        created_at = datetime.now(timezone.utc).isoformat()
